    ...
    class keyword_tracker:
        ...
        def run(self):
            ...
            try:
                page2 = None
                page3 = None       
                page2exist = soup.find("span", attrs={"class": "s-pagination-strip"})
                
                if page2exist:
                    page2 = page2exist.find("a", attrs={"aria-label": "Go to page 2"})["href"]
                    
                page3exist = soup.find("span", attrs={"class": "s-pagination-strip"})
                if page3exist:
                    page3 = page3exist.find("a", attrs={"aria-label": "Go to page 3"})["href"]
            ...
            for idx, page in enumerate(pages):
                t = threading.Thread(target=self.search_pages, args=[page, idx])
                t.start()
                threads.append(t)
        ...
        def get_top_brands(self, soup):
            ...
            try:
                for s in soup:
                    ...
                    if not asin:
                        link = soup.find("a", attrs={"class": "a-link-normal _bXVsd_link_36_Co"})
                        if link:
                            if link.get("href", None):
                                asins = link.get("href").strip().split("lp_asins=")[1].split("&")[0].split("%2C")
            except Exception as e:
                print(e)
            return asins
        ...
        def get_bottom_brands(self, soup):
            ...
            try:
                for s in soup:
                    asin = s.parent.get("data-asin", None)
                    if not asin:
                        link = soup.find("a", attrs={"class": "a-spacing-none a-link-normal _bXVsd_mainImageLink_1UpRh _bXVsd_link_gJc5l _bXVsd_hidden_L-XDK"})
                        if link:
                            if link.get("href", None):
                                asins.append(*link.get("href").strip().split("lp_asins=")[1].split("&")[0].split("%2C"))
            except Exception as e:
                print(e)
            return asins
        ...
        def get_video_ads(self, soup):
            ...
            try:
                for s in soup:
                    link = s.find("div", attrs={"class": "a-section sbv-video aok-relative sbv-vertical-center-within-parent"})
                    if link and link.find("a") and "lp_asins=" in link.find("a").get("href").strip():
                        asins.append(*link.find("a").get("href").strip().split("lp_asins=")[1].split("&")[0].split("%2C"))
            except Exception as e:
                print(e)
            return asins    
        def search_pages(self, page, idx):
            soup = BeautifulSoup(requests.get(page, headers=self.headers).content, "lxml")
            ...
            bottom_brands = self.get_bottom_brands(soup.find_all("div", attrs={"class": "celwidget pd_rd_w-2XDdr content-id-amzn1.sym.85014337-b4b1-4f3c-85f8-828d3b814280:amzn1.sym.85014337-b4b1-4f3c-85f8-828d3b814280 pf_rd_p-85014337-b4b1-4f3c-85f8-828d3b814280 pf_rd_r-182C8EY4Z98M4WCY4WP5 pd_rd_wg-5nAKj pd_rd_r-39c4e05c-4ece-48cc-8265-7065234a86da c-f"}))
            ...
            for link in links:
                ...
                is_best_seller = soup.find("span", attrs={"class": "a-badge-text"})
                if is_best_seller and "Best Seller" in is_best_seller.text:
                    best_seller = True
                    
                is_discounted = soup.find("a", attrs={"class": "a-size-base a-link-normal s-underline-text s-underline-link-text s-link-style a-text-normal"})
                if is_discounted:
                    discount = is_discounted.find("span", attrs={"class": "a-size-extra-large s-color-discount s-light-weight-text"})
                    if discount:
                        discount = discount.text.strip()
                        
                is_prime = soup.find("i", attrs={"class": "a-icon a-icon-prime a-icon-small"})
                if is_prime:
                    prime = True
                    
                is_limited_deal = soup.find("span", attrs={"class": "a-size-base a-color-price"})
                if is_limited_deal and "order soon." in is_limited_deal.text:
                    limited_deal = is_limited_deal.text
            
                is_editorial_pick = soup.find("span", attrs={"class": "a-badge-text"})
            ...
            for link in links_list:
                ...
                soup = BeautifulSoup(requests.get(self.base_url + link["link"], headers=self.headers).content, "lxml")
                t = threading.Thread(target=self.get_data, args=[soup, counter, link, idx])
                t.start()
                threads.append(t)
            for t in threads:
                t.join()
        ...
        def get_asin(self, soup):
            try:
                ASIN = None
                is_ASIN = soup.find_all("th", attrs={'class': 'a-color-secondary a-size-base prodDetSectionEntry'})
                for val in is_ASIN:
                    if val.string.strip() == "ASIN":
                        ASIN = val.parent.td.string
                        
                if not ASIN:
                    is_ASIN = soup.find("div", attrs={"id": "detailBullets_feature_div"})
                    if is_ASIN:
        ...
        def is_amazon_choice(self, soup):
            try:
                choice = False
                is_amzn_choice = soup.find("div", attrs={"class": "ac-badge-wrapper"})
                if is_amzn_choice:
                    choice = True
        ...
        def get_coupon(self, soup):
            try:
                coupon = None
                tag = soup.find("span", attrs={"class": "apl_m_font apl_message"})
                if tag:
                    coupon = coupon.span.string
                if not coupon:
                    tag = soup.find("span", attrs={"class": "promoPriceBlockMessage"})
                    coupon = tag.div.find("span", attrs={"class": "a-color-success"}).span.text.strip()
            except:
                coupon = None
        ...
        def get_delivery(self, soup):
            try:
                delivery = None
                tag = soup.find("div", attrs={"id": "mir-layout-DELIVERY_BLOCK-slot-PRIMARY_DELIVERY_MESSAGE_LARGE"})
        ...
        def get_tag(self, soup):
            ...
            try:
                if is_tag:
                    tag = is_tag.find('span', {'class': 'a-badge-text'}).text,
        ...
        def get_discount(self, soup, link):
            ...
            discount = None
            try:
                tag = soup.find("td", attrs={"class": "a-span12 a-color-price a-size-base"})
                if tag:
